...
export class KCCronService {
  ...
  async handleWeeklyRewards() {
    ...
    const brandAllowance = await this.settingService.getSetting(
      KCSettingKey.WEEKLY_REWARD_BRAND,
    );
    const influencerAllowance = await this.settingService.getSetting(
  ...
  private async awardBulkAllowance(
    manager: any,
    role: UserRole,
    amount: number,
  ) {
