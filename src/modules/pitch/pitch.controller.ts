...
export class PitchController {
  ...
  findSentPitches(@Req() req: any, @Query() query: PaginationQueryDto) {
    return this.pitchService.getInfluencerPitches(
      req.user.id,
      query.page,
      query.limit,
    );
  ...
  @Roles(UserRole.USER)
  @ApiOperation({
    summary: "List all pitches received by the current brand/user",
  })
  @ApiOkResponseEnvelope(Pitch, true)
  findReceivedPitches(@Req() req: any, @Query() query: PaginationQueryDto) {
    return this.pitchService.getTargetPitches(
      req.user.id,
      query.page,
      query.limit,
    );
