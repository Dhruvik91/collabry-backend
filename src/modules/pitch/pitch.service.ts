...
export class PitchService {
    ...
    async createPitch(influencerId: string, dto: CreatePitchDto) {
        const influencer = await this.userRepo.findOne({ where: { id: influencerId } });
    ...
    async getTargetPitches(targetId: string, page = 1, limit = 10) {
        const [items, total] = await this.pitchRepo.findAndCount({
            ...
            relations: ['influencer', 'influencer.influencerProfile', 'influencer.profile'],
            order: { createdAt: 'DESC' },
            skip: (page - 1) * limit,
            take: limit,
        });
    ...
    async getPitchById(id: string) {
        const pitch = await this.pitchRepo.findOne({
            where: { id },
            relations: ['influencer', 'influencer.influencerProfile', 'target', 'target.profile'],
        });
    ...
    async getAllPitches(page = 1, limit = 10) {
        const [items, total] = await this.pitchRepo.findAndCount({
            relations: ['influencer', 'influencer.profile', 'target', 'target.profile'],
            order: { createdAt: 'DESC' },
            skip: (page - 1) * limit,
            take: limit,
        });
