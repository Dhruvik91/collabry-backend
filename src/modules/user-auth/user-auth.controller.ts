...
import {
  MessageResponseDto,
  SuccessResponseDto,
} from "../../core/dto/message-response.dto";
import {
  AuthResponseDto,
...
export class UserAuthController {
  ...
  @ApiBadRequestResponseEnvelope(
    "Password mismatch or Admin role signup attempt",
  )
  ...
  @Post("login")
  @ApiOperation({
    summary: "Login for all user types (USER, INFLUENCER, ADMIN)",
  })
  @ApiOkResponseEnvelope(AuthResponseDto)
  @ApiUnauthorizedResponseEnvelope(
    "Invalid credentials or account suspended/unverified",
  )
  ...
  async createInfluencer(@Body() body: CreateInfluencerDto) {
    return this.auth.createInfluencer(
      body.email,
      body.password,
      body.confirmPassword,
      body.username,
  ...
  @Post("logout")
  @ApiOperation({
    summary: "Logout current user (client should clear stored JWT)",
  })
  ...
  @UseGuards(AuthGuard("google"))
  @ApiOperation({
    summary: "Handle Google OAuth callback and redirect with JWT",
  })
  async googleAuthRedirect(@Req() req: Request, @Res() res: Response) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const profile = (req as any).user as { email: string; name?: string };
    const result = await this.auth.upsertGoogleUser({
      email: profile.email,
      name: profile.name,
    });
    // Redirect with token as query (frontend should capture and store)
    const redirectUrl =
      process.env.GOOGLE_FRONTEND_REDIRECT_LINK ||
