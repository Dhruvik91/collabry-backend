...
export class ReferralService {
  ...
  async rewardReferral(referredId: string) {
    ...
    const referrerReward = await this.settingService.getSetting(
      KCSettingKey.REFERRAL_REWARD_REFERRER,
    );
    const referredReward = await this.settingService.getSetting(
  ...
  async getReferralStats(userId: string) {
    ...
    return {
      ...
      totalEarned,
      referralCode: (await this.userRepo.findOne({ where: { id: userId } }))
        ?.referralCode,
    };
  }
