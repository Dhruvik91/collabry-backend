...
describe("Sorting Verification", () => {
        ...
        ProfileService,
        {
          ...
          useValue: {
            createQueryBuilder: jest
              .fn()
              .mockReturnValue(mockInfluencerQueryBuilder),
          },
        },
        {
          ...
          useValue: {
            createQueryBuilder: jest
              .fn()
              .mockReturnValue(mockProfileQueryBuilder),
          },
        },
        {
  ...
  it("influencerService.searchInfluencers should order by completed collaborations count subquery and then createdAt", async () => {
    ...
    expect(influencerOrderByCalls[0].val).toBe(
  ...
  it("influencerService.searchInfluencers should filter by gender case-insensitively", async () => {
    await influencerService.searchInfluencers({
      page: 1,
      limit: 10,
      gender: "Female",
    } as any);

    const genderFilter = influencerAndWhereCalls.find((call) =>
      call.queryStr.includes("gender"),
    );
    expect(genderFilter).toBeDefined();
    expect(genderFilter.queryStr).toBe(
      "LOWER(influencer.gender) = LOWER(:gender)",
    );
