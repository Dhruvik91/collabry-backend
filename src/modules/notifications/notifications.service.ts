...
export class NotificationsService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    const subject =
      ...
      "mailto:support@kollabary.com";

    if (!publicKey || !privateKey) {
      this.logger.warn(
        "VAPID credentials missing in Environment configuration! Push notifications will not be active.",
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        "VAPID details successfully configured for Push Notifications.",
      );
    } catch (error) {
  ...
  async sendPushToUser(
    ...
    if (subscriptions.length === 0) {
      this.logger.debug(
        `No active push subscriptions found for user ID: ${userId}`,
      );
      return;
    }
    ...
    const promises = subscriptions.map(async (sub) => {
      ...
      try {
        ...
        this.logger.log(
          `Push notification sent successfully to user ${userId} on subscription ${sub.id}`,
        );
      } catch (err: any) {
        // If endpoint is expired or gone (HttpStatus 410 or 404), prune it
        if (err.statusCode === 410 || err.statusCode === 404) {
          this.logger.warn(
            `Subscription expired or gone (Status ${err.statusCode}). Pruning subscription ID ${sub.id}`,
          );
          await this.subscriptionRepo.delete(sub.id);
        } else {
          this.logger.error(
            `Failed to send notification to sub ${sub.id} for user ${userId}:`,
            err,
          );
        }
      }
