...
export class PublicService {
  ...
  private mapInfluencerSummary(collab: Collaboration) {
    return {
      ...
      fullName:
        ...
        collab.influencer?.user?.profile?.fullName ||
        collab.influencer?.user?.profile?.username ||
        "Creator",
      avatarUrl:
        collab.influencer?.avatarUrl ||
        collab.influencer?.user?.profile?.avatarUrl,
  ...
  async getInfluencerPublicData(
    id: string,
  ): Promise<PublicInfluencerProfileDto> {
    ...
    const calculatedTotalReach =
      influencer.totalFollowers ||
      Object.values(influencer.platforms || {}).reduce(
    ...
    return {
      ...
      username:
        ...
        influencer.user?.profile?.username ||
        "user",
  ...
  async getInfluencerPublicDataBySlug(
    slug: string,
  ): Promise<PublicInfluencerProfileDto> {
  ...
  async getBrandPublicData(id: string): Promise<PublicBrandProfileDto> {
    ...
    return {
      ...
      stats: {
        ...
        completedCollaborations:
          stats.completedCollaborations ?? collaborators.length,
      },
