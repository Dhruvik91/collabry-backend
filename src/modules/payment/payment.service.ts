...
export class PaymentService {
  ...
  async verifyPayment(userId: string, dto: VerifyPaymentDto) {
    ...
    if (initialOrder.user.id !== userId)
      throw new BadRequestException("Unauthorized payment verification");
    if (initialOrder.status === PaymentStatus.SUCCESS)
    ...
    if (Math.abs(paidAmount - Number(initialOrder.amount)) > 0.01) {
      this.logger.error(
        `Amount mismatch for order ${initialOrder.id}. Expected: ${initialOrder.amount}, Paid: ${paidAmount}`,
      );
      initialOrder.status = PaymentStatus.FAILED;
      await this.orderRepo.save(initialOrder);
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      order.metadata = {
        ...(order.metadata || {}),
        verify_api_response: paymentDetails,
      };
  ...
  async syncOrderStatus(userId: string, orderId: string) {
    ...
    try {
      const rzpOrder = await this.razorpayService.fetchOrder(
        order.razorpayOrderId,
      );
      const rzpPayments = await this.razorpayService.fetchOrderPayments(
        order.razorpayOrderId,
      );

      const successfulPayment = rzpPayments?.items?.find(
      ...
      const failedPayment = rzpPayments?.items?.find(
      ...
      if (failedPayment && order.status !== PaymentStatus.FAILED) {
        ...
        order.metadata = {
          ...(order.metadata || {}),
          rzp_sync_failure: failedPayment,
        };
        await this.orderRepo.save(order);
  ...
  async handleWebhook(payload: any, signature: string, rawBody?: string) {
    ...
    if (!webhookSecret || !signature || !rawBody) {
      this.logger.warn(
        "RAZORPAY_WEBHOOK_SECRET not configured or missing signature/body, skipping webhook verification",
      );
      return;
    }
    ...
    switch (event) {
          ...
          if (order && order.status !== PaymentStatus.SUCCESS) {
            ...
            if (paidAmountInPaise) {
              ...
              if (Math.abs(paidAmount - Number(order.amount)) > 0.01) {
                this.logger.error(
                  `Webhook Amount mismatch for order ${order.id}. Expected: ${order.amount}, Paid: ${paidAmount}`,
                );
                order.status = PaymentStatus.FAILED;
            ...
            void this.mailerService
              ...
              .catch((err) =>
                this.logger.error(
                  "Failed to send payment success email (webhook):",
                  err,
                ),
              );
          }
      ...
      case RazorpayWebhookEvent.PAYMENT_FAILED:
        ...
        await this.orderRepo.save(initialOrder);
        this.logger.log(
          `Payment failed for order ${razorpayOrderId}: ${initialOrder.metadata.failure_reason}`,
        );
        break;
      case RazorpayWebhookEvent.REFUND_PROCESSED:
        ...
        await this.orderRepo.save(initialOrder);
        this.logger.warn(
          `Refund processed for order ${razorpayOrderId}. Internal status updated to REFUNDED.`,
        );
        break;
      default:
  ...
  private async findOrderByIdOrRazorpayId(
    userId: string,
    orderId: string,
    relations: string[] = [],
  ) {
