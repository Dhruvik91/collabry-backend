...
export class AdminService {
  ...
  async getStatistics(filter?: AdminFinanceFilterDto): Promise<AdminStatsDto> {
    try {
      const [users, collaborations, verifications, reviews, growth, finance] =
        await Promise.all([
          this.getUserStats(),
  ...
  async getFinanceStats(
    filter?: AdminFinanceFilterDto,
  ): Promise<FinanceStatsDto> {
    ...
    const successRate =
      totalOrders > 0 ? (successOrders.length / totalOrders) * 100 : 0;
  ...
  private calculateDateRange(filter?: AdminFinanceFilterDto): {
    startDate: Date;
    endDate: Date;
  } {
    const now = new Date();
  ...
  private async getCollaborationStats(): Promise<CollaborationStatsDto> {
    ...
    const completionRate =
      totalCollaborations > 0
        ? Math.round((completedCollaborations / totalCollaborations) * 1000) /
          10
        : 0;
